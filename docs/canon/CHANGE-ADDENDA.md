...
## 18/09/2026 · SPEC-EXTRA-001.5 · publicar a linha publica o PLANO — BLOCKER
**Problema:** a proposta desenhou a fila de curadoria sobre a LINHA de serviço, mas a leitura (`planos_publicados`, `buscar_servico`) exige plano `publicado`, e o único chamador de publicação era `publicar_servico`. **Evidência:** 📊 juiz Fable sobre `3394836`: `grep publicar_plano` → 0 chamadores; ataque em memória com serviço `publicado` sob plano `proposto` → `nao_sabemos_ainda`. **Consequência:** o Founder publicaria 10 linhas pela tela e o segurado continuaria ouvindo "ainda não sabemos". **Autorização:** conserto único do laço (§5 ⑤) — publicar a linha publica o plano pai com o mesmo revisor, e a tela avisa.

## 18/09/2026 · SPEC-EXTRA-001.5 · o plano único da condição geral — ESSENCIAL
**Problema:** a proposta manda identificar o plano pelo NOME no texto da apólice (M-B4: nunca o "padrão da seguradora"). 📊 Medido depois da onda 1: **25 de 38** planos extraídos chamam-se "Plano único" — nome que apólice nenhuma escreve. **Consequência:** 66 % da base nunca seria alcançada, e o ELO da SPEC continuaria aberto com a base cheia. **Autorização:** D-E0015-01 (nota 85 × 40), com TRÊS travas juntas — um único plano publicado para seguradora × ramo × produto, vigente na emissão, e o processo SUSEP da apólice casado com o documento DAQUELE plano. Sem as três → `nao_sabemos_ainda`. Não é o "nível 1 por default" que M-B4 proíbe.

## 18/09/2026 · SPEC-EXTRA-001.5 · a vigência do plano vem do documento, não da rodada — ESSENCIAL
**Problema:** o extrator gravava `vigencia_inicio = date.today()`. 📊 Lente do dado: 38/38 planos com a data da rodada, enquanto `effective_from` dos 27 documentos-fonte vai de 2025-12-05 a 2026-08-06. **Consequência:** os dois UNIQUE do plano incluem a vigência — rodar a onda no dia seguinte duplicaria a base inteira, e a busca "pela condição vigente na emissão" (§7.2) responderia pela condição errada. **Autorização:** conserto único · D-E0015-07; documento sem vigência é PULADO.

## 18/09/2026 · SPEC-EXTRA-001.5 · a fila mostra a página, não um carimbo que sempre diz "não confere" — BLOCKER
**Problema:** a rota da fila hasheava a PÁGINA inteira e comparava com o `trecho_hash`, que é o hash do TRECHO (dialeto de medição, CLAUDE.md §9.4). **Evidência:** 📊 `fila(limite=4)` → `trecho_confere=False` em 4/4; `conferir_pagina(trecho_hash=…)` → 3/3 `trecho_nao_esta_na_pagina`. **Consequência:** quem cura veria "não confere" em toda linha certa e aprenderia a ignorar o aviso. **Autorização:** conserto único — campo removido; a fila devolve a página inteira com os termos do serviço grifados, e `conferido_na_proposta` vem do verificador, que é quem tinha o trecho.

## 18/09/2026 · SPEC-EXTRA-001.5 · negação tem de ser DO serviço — VALIOSA
**Problema:** o guarda de saída de `nodes.py` tratava qualquer `\bsem\b` como negação. **Evidência:** 📊 ataque do juiz: *"Tem carro reserva, sem custo adicional."* atravessava o guarda com a base dizendo `nao`. **Consequência:** uma afirmação errada chegaria ao segurado justamente no caso que o guarda existe para barrar. **Autorização:** conserto único — a negação passa a ser referida ao serviço (`não (tem|inclui|cobre)`, `sem (direito|cobertura)`, `não está incluído`), com par de controle.

## 18/09/2026 · SPEC-EXTRA-001.5 · exclusão de risco não vira "não" do plano — VALIOSA
**Problema:** o extrator lia *"riscos excluídos: inundação por transbordamento de rios"*, dentro da cobertura Vendaval/Granizo, e propunha `alagamento = nao` para o plano inteiro. **Evidência:** 📊 lente do dado, 2 das 6 linhas da amostra (HDI p. 88, Allianz p. 86); na base, `alagamento` tinha condicionado 5 · nao 3 · sim 3 sem regra que explicasse a divergência. **Consequência:** o segurado ouviria "não cobre" sobre um serviço que a seguradora presta por mão de obra hidráulica. **Autorização:** conserto único — regra no prompt e no verificador; as 3 linhas foram para `rascunho` com motivo, não apagadas.

## 19/09/2026 · SPEC-EXTRA-001.5.1 · o lote publicava 17 e dizia 23 — BLOCKER
**Problema:** `publicar_linhas_da_base.py:253` exigia o plano pai em `proposto`, mas `publicar_servico` publica o plano junto — a partir da 2ª linha do mesmo plano o pai já estava `publicado` e a linha era pulada com o motivo `plano_pai_publicado`, na mesma lista de `ja_em_publicado`. **Evidência:** 📊 o ensaio prometia 23 e o `--aplicar` entregou 17; a 2ª rodada mostrou `plano_pai_publicado 6`. **Consequência:** 6 linhas conferidas contra a página não chegaram ao cliente, e o relatório lia como sucesso. **Autorização:** conserto dentro do escopo (a unidade B é a publicação em lote); o relatório passou a separar PERDA de idempotência e a dizer *"🔴 N linhas conferidas NÃO chegaram ao cliente"*.

## 19/09/2026 · SPEC-EXTRA-001.5.1 · o acionamento não pode ser respondido com "deixa eu confirmar" — BLOCKER
**Problema:** a flag que impede a LLM de mentir sobre cobertura não distinguia PERGUNTAR de PEDIR, e trocava a resposta legítima do atendimento. **Evidência:** 📊 juiz da rodada 1 → 6 de 11 frases de pedido de guincho substituídas por *"vou confirmar e te respondo"*; controle sem a flag: 0/11. **Consequência:** o segurado parado na estrada perderia um turno inteiro. **Autorização:** porta de intenção pura na Skill (D-E00151-05), fora do texto da SPEC, registrada aqui.

## 19/09/2026 · SPEC-EXTRA-001.5.1 · `condicionado` não vira um "sim" liso — ESSENCIAL
**Problema:** com a base dizendo `condicionado`, o candidato *"Sim! Seu plano tem vidros"* passava e a condição sumia. **Evidência:** 📊 17 das 25 linhas que entraram em produção são `condicionado`; o juiz mediu 8/8 "sim liso" escapando antes da régua e 0/8 depois. **Consequência:** o segurado ouve um "sim" que o contrato dele não dá. **Autorização:** guarda novo em `nodes.py`, dentro do escopo da unidade C.

## 26/09/2026 · SPEC-117 · `ficha["apolice"]` passa a ter escritor — o aviso ao humano lia uma chave que ninguém preenchia — BLOCKER
**Problema:** a proposta pedia a apólice **no estado do caso**; não previa que o aviso à atendente humana já lia a apólice de outro lugar. **Evidência:** `human_handoff._linha_da_apolice` (`human_handoff.py:607`) lê `ficha_atendimento["apolice"]` e **nenhum escritor** existia — é a mesma classe do defeito registrado em **P-S117-04** para `["placa"]`, `["narrativa"]` e `["faltando"]`. **Consequência:** a apólice do caso passa a ser promovida à ficha durável por **um** escritor (📊 conferido pelo juiz: `attendance_ficha.py:438/440`, alimentado só por `novidades_da_apolice`, e `_gravar_apolice_do_caso` com 1 chamador), de modo que a pessoa que assume o caso recebe a apólice em vez de uma linha vazia. **Autorização:** licença do Founder para melhoria simples de efeito notório; blocker pelo teste do produto (protocolo §2: muda o que chega à corretora).

## 26/09/2026 · SPEC-117 · o guarda de família do serviço, no acionamento e na ficha — ESSENCIAL
**Problema:** o ramo e a seguradora "oficiais" passariam a **vencer** o palpite do modelo sem conferir se a apólice escolhida **serve o serviço pedido** — a proposta não previa guarda nenhum nesse ponto. **Evidência:** 📊 B3, apontado pelo juiz **e** pelo red team: pedido de **encanador** saía com `ramo='auto' insurer='allianz'`. E a causa de raiz, medida pelos dois laudos: `apolices_vigentes(contexto, ramo=…)` e `escolher_apolice` — as duas funções que o G5 prometia — tinham **ZERO chamadores em `backend/app`**; foram construídas e nunca ligadas (`specs-propostas/SPEC-117-LAUDO-JUIZ.md` §G5). **Consequência:** um guarda **único**, com três chamadores (acionamento, portal e ficha): o encanador passa a receber `residencial/porto`. Sem ele, o conserto criava um defeito **da mesma classe** que vinha corrigir, e pior — silencioso e com a autoridade de um dado "oficial" (CLAUDE.md §9.5). **Autorização:** conserto único do laço (protocolo §5 ⑤); blocker pelo teste do produto (muda a mensagem e o acionamento do segurado).

## 26/09/2026 · SPEC-117 · `attendance_ficha.LINHA_DO_CORREDOR_POR_FAMILIA` — a família crua não é o que o corredor entende — ESSENCIAL
**Problema:** a família de apólice que o sistema calcula é `resi`; a linha que o **corredor de acionamento** entende é `residencial`. Gravar a família crua na ficha deixaria a lista de campos obrigatórios **vazia, em silêncio** — e a proposta não previa tradução nenhuma. **Evidência:** 📊 o laudo do juiz registra o par no log do produto: `[APOLICE] ramo DIVERGENTE na ficha: modelo='resi' sistema='residencial' — vale o sistema` (`specs-propostas/SPEC-117-LAUDO-JUIZ.md:213`). **Consequência:** a tradução passa a existir **num lugar só**, `attendance_ficha.LINHA_DO_CORREDOR_POR_FAMILIA` (`attendance_ficha.py:271`), consultada por `nodes.py:2332`; família desconhecida devolve `""` e o sistema **não sobrescreve** o que o modelo pediu, em vez de gravar um valor que ninguém lê. **Autorização:** dentro do escopo da fatia da ficha, registrado aqui porque a proposta não previa a tradução; CLAUDE.md §9.4 (um valor medido num dialeto e aplicado noutro é um valor sobre outra coisa).

## 26/09/2026 · SPEC-117 · 4 arquivos de teste pré-existentes alterados, e a verdade vencida do teste de versão do corpus — VALIOSA
**Problema:** a assinatura nova (`company_id`, `papel`) teve de ser propagada aos chamadores, e quatro harnesses antigos stubavam o que agora é autoridade — eles precisavam passar a carregar os módulos **reais** (`policy_context`, `policy_facts`, `policy_data_provider`), sem dublê. Além disso, o teste que guardava a **versão do corpus** afirmava um número que a fatia F4a fez vencer (40 → 51 casos, versão 3 → 4). **Evidência:** 📊 o juiz conferiu **asserção por asserção** e classificou como *conserto de harness, não régua afrouxada* (`specs-propostas/SPEC-117-LAUDO-JUIZ.md` §"testes alterados"): `test_spec016_policy_intelligence` 94 → **94** · `test_spec016_e2e_stub` 22 → **22** · `test_infocap_policy_output_guard` 14 → **14** · `test_spec016_1_answer_quality` 41 → **43** (duas asserções **novas**, uma delas linha de controle); em `test_spec116_conserto_unico` a verdade vencida (`== 3`) foi **migrada**, não apagada. **Consequência:** nenhuma afirmação foi removida — no `answer_quality` os dicionários escritos à mão viraram saída do **construtor real**, e as afirmações do D6 continuam idênticas; o teste do corpus passa a guardar o **acompanhamento** (commit `46930ae`), não o número. **Autorização:** CLAUDE.md §9.3 (teste que guarda verdade vencida é pior que teste nenhum; a lição migra em vez de morrer) — harness e verdade, não redução de régua.
