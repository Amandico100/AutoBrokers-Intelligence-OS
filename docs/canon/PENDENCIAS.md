...
## P-098-UNIT-B-NA-SUITE · `test_098_builder_b_unit.py` (TestClient) dá 48 ERROR de setup dentro da suíte inteira e passa isolado e em pares
📊 06/09: suíte inteira 1.014 passed · 7 failed · 48 errors (`scratchpad/suite-inteira-098.txt`); isolado 80 passed; com o vizinho `test_098_builder_a_unit.py` 117 passed; com `test_a_atendente…` 86 passed. Dependência de ordem/estado global (mesma classe de P-093B-HARNESS). Dos 7 failed: 3 são guardas-script preexistentes (`test_todos_os_guardas_script_rodam`), 1 era o relatório da 098 ainda com placeholders, 3 passam isolados. **Destrava:** o fixture do TestClient construir o `FastAPI()` mínimo sem depender do módulo `app.main` já importado por outro teste; rodar a suíte com `-p randomly` uma vez para achar o vizinho. **Dono:** 🤖. **Custo de esquecer:** 48 errors que ensinam a ignorar a suíte.
...
📊 O PDF da HDI lista as franquias em prosa numa sequência sem dizer a cobertura; o cadastro só declara franquia em 3 das 10. A regra ancorada no cadastro casa 3 (2 confirmam o cadastro: Incêndio 350 = 350, Vidros 150 = 150; a 3ª é a divergência real: Danos Elétricos 600 × 550); as outras 3 (mínimos R$ 800, 650, 300) viram `Sinal("franquia_em_prosa_sem_dono")` — o corretor VÊ que existem (⚠️ isso só passou a ser verdade com o conserto único de 14/09: `avisos_da_apolice` no briefing, guarda `[GD2g]`; antes o sinal era escrito e ninguém lia — juiz fresco ④). **Destrava:** (a) o layout posicional do PDF (âncora de página/coluna) ou (b) 🧑 o Founder/seguradora declarar quais coberturas carregam franquia por seguradora/ramo. **Dono:** 🧑 / 🤖 (EXTRA-001.5). **Custo de esquecer:** se o sinal for descartado sem resolver, o corretor responde "não há franquia" para Ruptura de Tubulações, Vendaval e Equipamentos — e o segurado descobre no sinistro.

## P-E0011-TETO-DE-EVIDENCIA-60 · o teto de `evidence_items` subiu de 40 para 60; ninguém mediu outras seguradoras
📊 HDI 26, Allianz 25 itens (estruturados de todas as páginas antes dos fragmentos). ⚠️ Até o conserto único de 14/09 o teto de 60 valia só na EXTRAÇÃO — a entrega cortava em `[:20]` (lente do dado, blocker 1); agora `[:MAX_EVIDENCE_ITEMS]` nos dois lugares, guarda `[GC2g]` pelo caminho da produção. **Destrava:** medir sobre apólices reais de outras seguradoras (empresarial com 40 coberturas + franquias + 12 parcelas). **Dono:** 🤖 (EXTRA-001.5). **Custo de esquecer:** uma apólice grande volta a truncar em silêncio — e "inteira" deixa de ser verdade sem ninguém perceber.
...
## P-E0011-JUNCAO-SEM-TURNO-INALCANCAVEL · a trava da junção de `tool_calls` não tem como ficar vermelha
📊 14/09: a leitura de `tool_invocations` roda só no painel (`chat.py` devolve o widget antes de `_gerar`) e o painel recusa com 400 um turno sem `client_request_id`. A trava `if client_request_id` ao lado da junção é profundidade e nenhum guarda consegue exercitá-la. **Destrava:** 🤖 quando o widget ganhar turno tipado, ou quando o `client_request_id` virar opcional no painel — aí o par de controle da leitura vira exercitável. **Dono:** 🤖. **Custo de esquecer:** alguém remove a trava por parecer morta, e no dia em que o campo virar opcional o turno volta a colher a sessão inteira como se fossem as ferramentas dele.

## P-E0011-CACHE-DOCUMENTAL-SEM-CONEXAO-REAPROVEITAVEL · os PDFs já guardados perdem o hit depois do Implantar
📊 14/09: o nome guardado passou a carregar a conexão (`infocap-policy-<locator>-c<conexao>-<conteudo>.pdf`). Os arquivos escritos antes continuam encontráveis por quem não declara conexão, mas o conector SEMPRE declara — a primeira leitura de cada apólice depois do deploy baixa o PDF de novo. **Destrava:** 🤖 medir quantas linhas de `documents` têm o prefixo antigo e decidir entre backfill de nome ou deixar expirar. **Dono:** 🤖. **Custo de esquecer:** uma onda de downloads no primeiro dia depois do Implantar, sem ninguém saber de onde veio.
...
## P-E0014-22 · o índice cliente→seguradora expira em 420 s no ramo `seguradora_encerrou`
📊 17/09 (confirmação, P-b): quando o prazo vence pelo Vigia, `_segurar_ou_desistir` reindexa com `intervalo × (max+2) + 3600`; quando quem fecha é a **seguradora**, o ramo de `handle_insurer_message` guarda a `espera_vencida` mas **não** estende o índice — ele morre com o TTL original de `60 × 4 + 120 = 420 s`. Depois disso a resposta tardia do segurado cai no chat comum, e o agente a trata como mensagem qualquer. **Destrava:** estender o índice também nesse ramo (o motor é PURO e não faz IO — o reindex tem de sair do roteador, onde a sessão é gravada). **Dono:** 🤖. **Custo de esquecer:** o segurado responde depois de 7 min e o dado não chega à ficha do acionamento.

## P-E0014-23 · valor multi-token repartido em duas mensagens é recusado pela prova de origem
📊 17/09 (confirmação, P-c): `valor_tem_origem` exige a sequência de tokens dentro de **uma** linha (foi assim que o B2 fechou o furo do "casa as"). Se o segurado escreveu *"em frente a"* numa mensagem e *"padaria São Jorge"* na seguinte, o Cérebro não prova a origem e o valor é **recusado** — cai em perguntar ao segurado. ⚠️ **Falha para o lado seguro**: nunca vira byte errado à seguradora. **Destrava:** decidir se vale aceitar a sequência atravessando mensagens CONSECUTIVAS do mesmo autor (e o custo: a janela de colagem volta a existir). **Dono:** 🤖. **Custo de esquecer:** uma pergunta a mais ao segurado de vez em quando.
...
## P-E0014-19 · a resposta TARDIA do segurado não REABRE o acionamento
📊 17/09: o corredor só sabe reentrar quando **alguém da seguradora fala** (`pode_reentrar_em_fase_humana`); não existe caminho para nós reabrirmos uma URA fechada, e `pode_retomar` existe justamente para impedir um SEGUNDO acionamento. Por isso a resposta que chega depois do prazo, com o caso já entregue a uma pessoa, é **guardada** (ficha + `work_events`, `retomada=guardada`) em vez de levada. **Destrava:** decidir se um re-acionamento automático é aceitável (risco: dois prestadores) ou se a pessoa que tem o caso deve receber a resposta por uma linha na porta única. **Dono:** 🤖 + 🧑 (é decisão de produto). **Custo de esquecer:** o segurado responde, vê "já anotei" e ninguém leva o dado à seguradora.
...
📊 20/09/2026: o `/health` no ar tem 41 chaves em `codigo` e nenhuma sobre `JANELA_SILENCIO_EXCECOES`; a EXTRA-001.7 acrescentou a CONTAGEM (`excecoes_da_janela_tamanho`, `excecoes_da_janela_fora_do_teste`, nunca o número). Até o Implantar, a trava "exceções só com número de teste" sai NÃO CONFERIDA e reprova. **Destrava:** clicar Implantar em smith-api e rodar `python scripts/conferir_o_que_esta_no_ar.py --ligar` de novo. **Dono:** 🧑. **Custo de esquecer:** o checklist nunca fica verde; ou pior, alguém o ignora.

## P-E0017-03 · 🔴 três dimensões do atendimento NÃO TÊM ESCRITOR — sem eles o bloco "atendimento" nunca ganha nota, nem depois do piloto
📊 20/09/2026 (investigação por código + `medir_o_piloto.py`): **apólice certa em 1 rodada** — só existe o booleano sobrescrito `conversations.ficha_atendimento.apolice_confirmada`, sem histórico de rodadas e sem `work_events`; **sabe calar** — o produto grava QUANTAS vezes calou (`agent_activities`), nunca se o silêncio era o certo; **fala como humano** — exige avaliador lendo a conversa. Das 6 dimensões do atendimento, só 3 têm fonte durável (coleta e age · aciona · sabe pedir ajuda), e a régua só publica a nota do bloco com a maioria avaliada. **Destrava:** (a) gravar um evento `apolice.fixada` com o nº da rodada e se houve correção depois (SPEC pequena, toca o caminho do atendimento → CRÍTICO pelo piso); (b) um botão "o agente devia ter falado / devia ter calado" na Ficha, gravado por conversa; (c) "fala como humano": amostragem avaliada por gente (Saionara/Regina, 10 conversas por dia de piloto) lançada num campo — nunca por SELECT. **Dono:** 🤖 (a, b) · 🧑 (c). **Custo de esquecer:** o piloto de 3 dias roda e metade das notas do §0 do diagnóstico continua palpite.

## P-E0017-04 · o chat principal não registra FALHA nem liga pergunta a apólice
📊 20/09/2026: `conversation_logs.status` é o literal `"success"` (`app/agents/nodes.py:2086`) em 567 de 567 linhas da base — a coluna não consegue discordar, e resposta que não saiu não deixa linha; nada liga a pergunta à apólice usada nem à pergunta anterior. Por isso "confiabilidade", "apólice certa", "uma rodada" e "completude" do chat saem NÃO AVALIADA. E `response_time_ms` é o tempo do MODELO (`llm_response_time_ms`), não o que a pessoa esperou. **Destrava:** gravar a falha (status ≠ success + classe do erro), o tempo de ponta a ponta e a apólice resolvida por turno. **Dono:** 🤖. **Custo de esquecer:** o "Não consegui gerar a resposta" de 17/09 continua invisível para qualquer régua.

## P-E0017-05 · `handoff.teto_de_lembretes` conta VARREDURAS do vigia, não casos
📊 20/09/2026: 162 eventos em cada corretora de 16 a 20/09 (distribuição diária diferente: coincidência, conferido por `company_id`). `app/tasks/handoff_watchdog.py:334` emite a cada varredura, e o `payload_redacted` não tem id de conversa nem `work_run_id`. **Destrava:** carregar o id curto da conversa no payload e emitir 1× por caso. **Dono:** 🤖. **Custo de esquecer:** o diário enche de ruído (324 linhas em 5 dias) e ninguém sabe quantos casos estão parados.

## P-E0017-06 · o porteiro do painel é fail-OPEN sem dizer que não conferiu
📊 20/09/2026: `app/api/porteiro_do_agente.py:83-85` e `:103-105` devolvem `{"pode": true}` quando a leitura do banco falha — resposta idêntica à de "está tudo certo". O checklist novo NÃO herda isso (o que não se confere reprova), mas o botão de ligar do painel herda. **Destrava:** acrescentar `"conferido": bool` à resposta e a tela avisar. **Dono:** 🤖. **Custo de esquecer:** numa indisponibilidade de banco a corretora liga o agente sem destino nem canal conferidos.

## P-E0017-07 · o motor do resumo das 19h transforma diário ilegível em ZERO medido
📊 20/09/2026 (red team): `os_modelos_do_grupo.contagens_do_dia` tem `except → return c` com todos os campos em 0 e `truncou=0`; e `ler_paginado` devolve `([], True)` em exceção, com a mesma marca de "parou no teto". A medição do piloto se defende do lado dela; o resumo das 19h, não: num dia de banco instável a corretora leria "Sem acionamentos hoje". **Destrava:** o motor distinguir `nao_lido` de `truncou` e o resumo dizer "não consegui ler o dia". **Dono:** 🤖. **Custo de esquecer:** um número falso com cara de medição no grupo da corretora.

## P-E0017-08 · leitura de `POLICY_INTELLIGENCE_V2` duplicada, com listas diferentes
📊 20/09/2026 (investigador): `app/core/feature_flags.py:24 env_ligada` aceita `sim`; `app/agents/nodes.py:113` relê a mesma env com lista própria SEM `sim`. `POLICY_INTELLIGENCE_V2=sim` liga numa e não na outra. **Destrava:** `nodes.py` chamar `env_ligada`. **Dono:** 🤖. **Custo de esquecer:** meia flag ligada, e o canário mede o comportamento antigo sem ninguém entender por quê.
...
## P-E0017-12 · 🔴 a página de Corredores do painel não trava nada: o motor não lê `tenant_corridors`
📊 20/09/2026 (`grep tenant_corridors|corridor_enabled|is_enabled backend/app` → 4 linhas, todas comentário): Ativo = Pausado = Disponível para o atendimento real; `dispatch_router.py:3456 get_playbook` escolhe por seguradora sem consultar a corretora. 📊 SELECT: Resulta 1 ativo + 1 pausado, AutoFleet 14 ativos, Amandus nenhum. A corretora que PAUSA um corredor acredita que o robô parou de acionar aquela seguradora — e não parou. **Destrava:** o motor consultar `tenant_corridors` (pausado ⇒ handoff com dossiê), com teste de 2 corretoras; e decidir o padrão de corretora nova (tudo ativo × nada ativo). **Dono:** 🤖 (cabe na 001.8 ou na 001.9). **Custo de esquecer:** promessa de tela que o produto não cumpre, para toda corretora que entrar.

## P-E0017-13 · `ATTENDANT_INBOUND_ALLOWLIST` é GLOBAL: um ensaio numa corretora cala os clientes de todas as que tiverem agente ligado
📊 20/09/2026: descarte em `app/api/webhook.py:1045`, variável de processo, sem corretora. Hoje não causa perda porque todos os canais ativos são `purpose='observer'` e o `observer_tap` consome o evento antes (agentes desligados). No dia em que houver uma corretora real com agente ligado, preencher a lista para um ensaio descarta em silêncio todo segurado dela. **Destrava:** allowlist por corretora (coluna/config por `company_id`) e o checklist de ligar avisando "há corretora ligada + allowlist preenchida". **Dono:** 🤖 (001.8). **Custo de esquecer:** um dia inteiro de clientes ignorados sem log.
