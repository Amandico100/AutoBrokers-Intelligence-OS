...
**Marcha fixada, não rediscutível: PADRÃO · 1 lente + 1 juiz fresco + canário vivo · 💭 10–14 h · ≤ 12 guardas
novos.** Não amplie o teto de guardas. Não monte três lentes para "caprichar". 🔴 **E leia a §0.5 da proposta antes
de aceitar essa marcha:** a conta de risco dá 7 e a marcha é PADRÃO — a divergência tem de sair **escrita** no seu
card, com o piso aplicado nos dois pontos onde ele dispara.
...
```
docs/canon/specs/SPEC-EXTRA-001.5-o-agente-sabe-o-que-cada-plano-cobre.md   ← a SPEC DEFINITIVA
...
- Ler o corpus normativo e as fontes arquivadas no MinIO — são documentos **públicos** de seguradora.
- **Baixar condições gerais e manuais de assistência de sites públicos e do registro público da SUSEP (REP2)**,
  pelo caminho que já existe (`insurance_corpus._buscar`). Isto é leitura de documento público.
- Chat `core` do painel da Resulta pela conta do Founder, com as perguntas do corpus.
- Uma conversa de atendimento com **TESTE-A** (e **TESTE-B** se o caso exigir dois interlocutores), com o agente
  habilitado **apenas** para esses números — o mecanismo de exceção já existe (commit `05f46a9`,
  `JANELA_SILENCIO_EXCECOES`). **Não crie outro.**
- Rodar a suíte, mutações em cópia, e as **duas** migrations da §11 **depois** do gate.
...
3. **Entrar em portal de seguradora com login, acionar assistência, abrir chamado, alterar apólice.** Nada desta
   SPEC precisa disso.
...
**Investigador, pesquisador, aquecimento e juiz continuam read-only e sem envios.** Inclua esta fronteira em
...
- 🔴 **Esta SPEC depende da EXTRA-001.1** (a porta `PolicyDataProvider` e a chave canônica). Ela é **trilho
  paralelo**: abre quando o BLOCO A da 001.1 fechar. **Você não edita `backend/app/providers/policy_data_provider.py`**
  — dois escritores no mesmo arquivo é o que a coesão do protocolo §3.4 proíbe. Se a 001.1 ainda não entregou, meça
  o que existe e escreva o contrato mínimo de que precisa.
...
- O diagnóstico está em `docs/canon/DIAGNOSTICO-PILOTOS-E-PLANO-EXTRA-2026-09-12.md`. **As suas seções são §1.3,
  §3 (bloco 001.5), §7.3, §7.6 e §12.1. Leia essas, não o arquivo inteiro** — e saiba que **três números dele não
...
- `docs/canon/FOUNDER-DECISIONS.md` — só as linhas `D-PILOTO-01, 08, 11, 14, 16, 20`.
- `docs/canon/PENDENCIAS.md` — 🔴 **por número**: `P-PILOTO-04`, `P-PILOTO-20`. **Nunca o arquivo inteiro**
  (📊 562 KB).
...
3. **BLOCO 0 — converter medindo** (§4 da proposta). Reabra **cada** `arquivo:linha` do research pack §1 e
   **rerode as 7 medições** do §2. Divergiu? corrija na SPEC definitiva **e anote**. O seu número vence.
4. 🔴 **A primeira prova do BLOCO 0 é esta, e é um comando, não uma leitura:** rode o extrator documental sobre um
   PDF de apólice do acervo **com** e **sem** `susep` em `_BOILERPLATE_RE`
   (`policy_document_evidence_service.py:208`) e conte os processos recuperados. 📊 Esperado: **0 com o filtro,
   ≥1 sem**. É isso que transforma a onda 2 de intenção em conserto.
5. **Meça o ELO, não duas pontas** (protocolo §0.3): a afirmação-título é *"o agente responde genérico PORQUE não
   há plano estruturado"*. Meça A (as respostas genéricas do acervo), meça B (0 linhas de plano em qualquer tabela)
   e **meça que B chega em A** — rodando a Skill **atual** sobre 10 perguntas reais de "carro reserva" e mostrando
...
1. 🔴 **AFIRMAÇÃO DELIBERADAMENTE FALSA DO EXERCÍCIO, assinada pelo orquestrador:** *"A ligação apólice →
   condições gerais pelo processo SUSEP já casa hoje; a SPEC só precisa consumi-la."*
   Refute com `grep` e mostre **a linha que bloqueia o caminho**, com o comando que a prova.
2. 🔴 **AFIRMAÇÃO DELIBERADAMENTE FALSA DO EXERCÍCIO, assinada pelo orquestrador:** *"O corpus de condições gerais
   ...
   `curation_status`."*
   Refute as **duas** metades, com a consulta e com `arquivo:linha`.
...
5. Apagar `assistance_policy.py` é o conserto? Diga **o que mais cai junto** e por que a regra do guarda muda em
   vez de morrer.
6. *"Não sabemos ainda"* e *"a fonte não retornou"* são a mesma coisa para o usuário? Explique a diferença **e o
   que cada uma faz na tela e na contagem**.
7. A condição geral **atual** da seguradora serve para responder sobre uma apólice de 2023? Cite a frase do
   próprio código que responde isso, com caminho.
...
9. Ligar `citations: true` na API resolve a proveniência? Diga o que exatamente a documentação **garante** — e o
   que ela **não** garante.
10. A migration desta SPEC é rotina de PADRÃO? Diga **qual das duas** dispara o piso CRÍTICO, por quais dois
    motivos somados, e onde o DDL real tem de ser lido.
...
12. **Liste o que você NÃO entendeu ou não conseguiu provar** ("entendi tudo" reprova) **e ache um defeito material
    que a proposta não aponta** — ou diga exatamente onde procurou e não achou. Entregue a **nota** e o **card**
    que você aplicaria.
...
|---|---|
| **A chave** | módulo único de leitura; `tokio` e `tokio_marine` resolvem para a mesma; o que não casa sai `UNKNOWN` **listado**, nunca omitido |
| **As tabelas** | `insurer_assistance_plans` (com **`nivel`**) + `insurer_assistance_services`, com **documento-fonte e página obrigatórios por CHECK do BANCO**, e `publicado` exigindo revisor |
| **A Skill** | `cobertura_e_assistencia`: apólice vigente do ramo → PDF → produto/plano → linha da tabela → prosa da condição geral, nos **cinco estados**, com origem escrita |
| **Os estados** | 🔴 `nao_sabemos_ainda` **nunca** vira "não cobre"; `fonte_indisponivel` tem texto próprio e outra coluna na tela |
| **O gancho** | só aparece se o plano superior **existir na tabela** e cobrir o serviço. **Sem preço** — cotação é outra SPEC |
| **O fallback** | `assistance_policy.py` continua existindo, **marcado**, e o guarda de `nodes.py:307` muda de regra sem morrer |
| **As três ondas** | onda 1 das CGs dos ramos dos pilotos, com **revisão humana antes de publicar**; onda 2 o elo SUSEP; onda 3 as ausentes de maior carteira, com `manual_de_assistencia` |
| **A busca** | `doc_kind` filtra de verdade (índice criado **antes** da ingestão) e a procedência **volta como campo**, não como texto |
| **A tela** | cobertura por **seguradora × ramo** (nunca por linhas) + a fila de curadoria com trecho e página |
| **Provas** | 12 guardas + o canônico de isolamento, cada um **vermelho** na sua mutação, em cópia, em subprocesso |

🔴 **Não crie:** segundo pipeline de ingestão, segunda coleção de Qdrant, segunda fila de curadoria, segundo
catálogo de seguradoras, segundo caminho de leitura documental, segundo registro de chamada de ferramenta, segunda
porta de apólice (CLAUDE.md §5).

---

## 8. Execução AAA opção B — PADRÃO, sem desperdício e sem atalho

**PADRÃO nesta SPEC:** desenhista antes do código · builder por unidade coesa · verificador mecânico ·
**UMA lente — "produto + DADO"**, que é a que o protocolo §5 ④ exige quando o outcome é um dataset: ela
**reconstrói a base sobre o dado real (SELECT)** e pergunta se ela diz a verdade · conserto · **UM JUIZ FRESCO**,
contexto limpo, que confirma o conserto **e** audita o dado (§6.1). 🔴 **Juiz retomado é proibido** (§5.3).

**Coesão e paralelismo:** **A é arquivo-hub — um dono, primeiro, sozinho.** B e C em série sobre o contrato de A.
**D e E são disjuntas e correm em paralelo.** Nenhum outro escritor simultâneo.

**Bateria:** ≤ 12 guardas novos, todos sobre o **motor** e o **acervo real**. 🔴 **Proibido teste que reimplementa a
regra** (CLAUDE.md §9.4): o teste chama a tool e o banco, nunca um regex sobre a mesma tabela que o código lê.
**E toda bateria precisa de linha de controle** (§9.2) — nesta SPEC são três, e estão nomeadas: pergunta
não-assistência, nível máximo sem gancho, e `susep` de volta no boilerplate exigindo **ZERO**.
...
📊 Build verde com 287 rotas já conviveu com o produto inteiro devolvendo 500. **A tela de §8 muda o front.**

**Canário vivo é obrigatório**, com os 8 casos da §12.2 — inclusive os casos **5, 7 e 8**, que são as linhas de
...
timeout · não deixe agente escrevendo durante mutação ou gate final · salve conserto completo e retomável antes do
próximo.

**Orçamento:** 💭 0,9–1,5 M tokens; faixa de relógio 💭 10–14 h. **Faixa, nunca promessa** (§9.2). Se a janela
...
📊 As páginas seguem a convenção `p-home · p-s094 · p-s0941 · p-s097 · p-s098 · p-extra001 · p-pilotos · p-proto ·
p-fila`. A nova é **`p-extra0015`**, com item de navegação e linha na home.
...
Atualize **a cada bloco fechado**, não só no fim. Use **aliases** dos números de teste. **Estar na `main` não é
estar no ar.** Registre a fila em `INDICE-DE-SPECS.md`, `ESTADO-DAS-SPECS.md` e `EXECUTION-MASTER-PLAN.md`. A
família **EXTRA-001.x** passa pelos mesmos guardas — se um indexador só reconhecer `SPEC-0NN`, **adapte o
...
ser entregue — não force.

**Implantar:** `smith-api` **e** `smith-web` (a tela de Conhecimento muda o front). Se depender do clique do Founder
no EasyPanel, deixe tudo pronto e o clique como **único** ato final. Não contorne por API. **A ordem dos serviços
sai do contrato medido desta SPEC**, não copiada do histórico.
...
6. **Caixa do Founder** — o que só ele faz: colar as perguntas no chat da Resulta; mandar a mensagem do TESTE-A;
   🔴 **revisar e publicar as primeiras linhas da fila de curadoria** (ou indicar quem revisa — é a única
   dependência humana que **bloqueia o canário completo**; 💭 uma seguradora, um ramo, ~10 linhas já destravam);
   o clique do EasyPanel se houver.
7. **Handoff:** esta SPEC é **trilho paralelo**. A fila principal segue pela ordem do diagnóstico §12.1.
   **Não execute nenhuma outra agora.**
