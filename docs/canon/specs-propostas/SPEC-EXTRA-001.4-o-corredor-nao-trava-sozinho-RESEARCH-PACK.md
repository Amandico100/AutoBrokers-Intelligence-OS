...
| **R43** 🔴 | *"Por falta de contato, estou encerrando"* | `grep -ic "falta de contato"` → **0 nos 16**. `grep -ic "estou encerrando"` → **3, só em bradesco-auto**, e a redação é *"Bom, estou encerrando este atendimento por falta de interaç…"* — **que a regex de `:2548` já casa** |
| **R44** ✅ | *"Vou transferir seu caso para um especialista"* | **42 ocorrências literais**: allianz-residencial 31, allianz-auto 11. ⚠️ e 3 falsos positivos vizinhos na allianz-residencial: *"Previsão de chegada do especialista"* — é o **prestador**, já catalogado em `NAO_E_FRONTEIRA` |
...
2. **Qual é a redação real da frase de encerramento de setembro** (R43): o corpus não a tem, e a variante que tem já é coberta. Sai de `observed_events`, por consulta.
...
8. **Se `work_waits` tem leitor** para o caso "pergunta ao segurado", ou se o Vigia basta.
...
| a frase de encerramento | consulta a `observed_events`, paginada | que a frase do enunciado existe (R43) |
...
6. *"A âncora `insurer_closed` não cobre encerramento por falta de interação."* — leia `:2548` inteira. (R22/R43.)
