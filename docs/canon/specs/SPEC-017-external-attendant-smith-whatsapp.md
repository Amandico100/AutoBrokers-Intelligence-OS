...
| S17-12 | **Multi-número por corretora** (Founder 2026-07-03): `integrations.purpose` (`attendance` padrão · `auxiliary:<slug>` · `dispatch`) — cada função tem SEU WhatsApp com instância Evolution, webhook token e agente vinculado próprios, no MESMO seam (zero cérebro duplicado). Auxiliar que envia mensagens (cobrança/leads) conecta o próprio QR na configuração do Auxiliar (Onda 4). Atendimento nunca compartilha número com disparo. |
| S17-13 | **Vários atendentes humanos**: o WhatsApp é UM por função; a distribuição para humanos acontece na fila de Atendimentos (claim da conversa por qualquer humano da equipe, estados HUMAN_REQUESTED/HUMAN_ACTIVE do modelo V7) + espelhamento das conversas no dashboard — fases P5. |
