...
| **`DISPATCH_FINALIZE_LIVE_PLAYBOOKS`** | smith-api | ⚠️ **1 corredor só** | é de propósito, e é bom: **só aquele corredor** termina o chamado de verdade; todos os outros vão até o fim e **cancelam**. Para o bloco C.1 (que recusa antes da confirmação) tanto faz | se você quiser um chamado real em outra seguradora, precisa acrescentar o nome dela aqui — senão vai parecer que funcionou e nada foi aberto |
