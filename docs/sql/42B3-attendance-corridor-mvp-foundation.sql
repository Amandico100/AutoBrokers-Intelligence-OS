-- 42B3 — Attendance/Corridor MVP Database Foundation
-- Projeto: AutoBrokers Intelligence OS
-- Status: SQL controlado para Supabase SQL Editor
-- Criado a partir da SPEC-005, SPEC-006 e 42B3P.
--
-- O QUE ESTE SQL FAZ:
-- 1) Cria 4 tabelas mínimas:
--    - public.attendance_cases
--    - public.corridor_templates
--    - public.corridor_runs
--    - public.dispatch_packets
-- 2) Ativa RLS e policies multi-tenant.
-- 3) Cria índices.
-- 4) Cria seeds globais:
--    - Família Allianz Residencial / Assistência
--    - Subcorredor Eletricista MVP
--
-- O QUE ESTE SQL NÃO FAZ:
-- - Não altera runtime Smith.
-- - Não altera RAG.
-- - Não altera prompts.
-- - Não altera agentes.
-- - Não altera WhatsApp.
-- - Não cria envio real.
-- - Não ingere documentos.
-- - Não mexe em conversations/messages.
--
-- RECOMENDAÇÃO:
-- Rode primeiro a SEÇÃO 0.
-- Se a preflight estiver OK, rode a SEÇÃO 1 completa.
-- Não rode a SEÇÃO 3 (rollback) salvo se precisar desfazer.

-- ============================================================
-- SEÇÃO 0 — PREFLIGHT / INTROSPECTION READ-ONLY
-- ============================================================

-- 0.1 — Tabelas base necessárias
select
  'required_table_check' as section,
  t.table_name,
...
-- 0.2 — Colunas críticas em users_v2 e companies
select
  'required_columns_check' as section,
  table_name,
  column_name,
  data_type,
  is_nullable
...
where table_schema = 'public'
  and (
    ...
    or (table_name = 'users_v2' and column_name in ('id','company_id'))
    or (table_name = 'conversations' and column_name in ('id','company_id','user_id','session_id','status','channel','agent_id','user_phone','human_handoff_reason'))
    or (table_name = 'messages' and column_name in ('id','conversation_id','role','content','type','sender_user_id'))
    or (table_name = 'approval_requests' and column_name in ('id','company_id','subject_type','action_type','status','risk_level','request_payload','preview','approval_result'))
  )
order by table_name, ordinal_position;

-- 0.3 — Verifica se as 4 novas tabelas já existem
select
  'new_tables_existing_check' as section,
  t.table_name,
...
-- 0.4 — Verifica trigger function de updated_at
select
  'updated_at_function_check' as section,
  exists (
    select 1
    from pg_proc p
...
-- 0.5 — Verifica constraints atuais de messages.role; decisão MVP = NÃO alterar
select
  'messages_role_constraints' as section,
  conname,
...
-- 1.7 — Seeds globais: Allianz Residencial Family
with updated as (
...
-- 1.8 — Seed global: Allianz Residencial / Eletricista MVP
with updated as (
  update public.corridor_templates
  set
    scope = 'global',
...
-- ============================================================
-- SEÇÃO 2 — VERIFICAÇÃO PÓS-DDL
-- ============================================================

-- 2.1 — Tabelas novas
select
  'new_tables_created' as section,
  table_name
from information_schema.tables
where table_schema = 'public'
  and table_name in ('attendance_cases','corridor_templates','corridor_runs','dispatch_packets')
order by table_name;
...
where schemaname = 'public'
  and tablename in ('attendance_cases','corridor_templates','corridor_runs','dispatch_packets')
order by tablename;

-- 2.3 — Policies
select
  'policies_check' as section,
  schemaname,
  tablename,
  policyname,
  cmd,
...
-- 2.4 — Seeds globais
select
  'corridor_seed_check' as section,
  id,
  scope,
  company_id,
  corridor_key,
  ...
  status_operacional,
  is_active,
  jsonb_array_length(phases) as phases_count,
  jsonb_array_length(required_slots) as required_slots_count,
  jsonb_array_length(guardrails) as guardrails_count,
  jsonb_array_length(golden_tests) as golden_tests_count
from public.corridor_templates
where corridor_key = 'allianz_residential_assistance'
order by subcorridor_key nulls first;

-- 2.5 — Índices principais
select
  'index_check' as section,
  tablename,
  indexname
from pg_indexes
where schemaname = 'public'
...
-- ============================================================
-- SEÇÃO 3 — ROLLBACK MANUAL, NÃO RODAR POR PADRÃO
-- ============================================================
-- Para desfazer completamente esta fundação, rode manualmente:
--
-- begin;
...
--
-- Observação:
-- O rollback remove as 4 tabelas e seus seeds, policies, triggers e índices.
-- Não toca em conversations/messages, approval_requests, Vault, RAG, agentes ou prompts.
