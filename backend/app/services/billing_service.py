...
# ===== BALANCE CHECK CACHE =====
# Cache de has_sufficient_balance por company_id
# Formato: { company_id: (result: bool, timestamp: float) }
_balance_cache: Dict[str, Tuple[bool, float]] = {}
_BALANCE_CACHE_TTL = 30  # segundos


def _get_cached_balance(company_id: str) -> Optional[bool]:
    """Retorna resultado cacheado se ainda válido, None se expirado."""
    if company_id in _balance_cache:
        result, cached_at = _balance_cache[company_id]
        if time.time() - cached_at < _BALANCE_CACHE_TTL:
            return result
        # Expirado, remover
        del _balance_cache[company_id]
    return None


def _set_cached_balance(company_id: str, result: bool):
    """Cacheia resultado do balance check."""
    _balance_cache[company_id] = (result, time.time())
...
def invalidate_balance_cache(company_id: str):
    ...
    if company_id in _balance_cache:
        del _balance_cache[company_id]
        logger.debug(f"[BILLING CACHE] Invalidated cache for {company_id}")
...
class BillingService(BillingCore):
    ...
    def has_sufficient_balance(self, company_id: str, estimated_cost: Decimal = Decimal("0.01")) -> bool:
        """
        Verifica se cliente pode usar o serviço (com cache de 30s).

        Cache é seguro porque:
        - Billing roda de 5 em 5 min (saldo já está defasado)
        - Pior caso: 30s de mensagens extras antes do bloqueio
        - Cache é invalidado quando subscription muda (cancel, past_due)
        """
        # 1. Checar cache
        ...
        # 2. Sem cache, fazer as queries normais (herda do BillingCore)
        result = super().has_sufficient_balance(company_id, estimated_cost)

        # 3. Cachear resultado
        _set_cached_balance(company_id, result)
    ...
    def _send_cancellation_email(self, company_id: str, plan_id: Optional[str]) -> None:
        ...
        try:
            ...
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                ...
                <p style="margin-top: 30px;">Atenciosamente,<br>Equipe AutoBrokers</p>
            </div>
            """
