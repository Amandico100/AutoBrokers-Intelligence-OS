...
async def check_widget_rate_limit(
    ...
    try:
        ...
        # Fail-open: se o banco falhar (pool esgotado, timeout, etc.)
        # permite a request passar em vez de bloquear usuários legítimos.
        # O rate limit é uma proteção secundária — não deve derrubar o serviço.
        logger.error(f"[WIDGET SECURITY] Rate limit check failed (allowing request): {e}")
        return True
