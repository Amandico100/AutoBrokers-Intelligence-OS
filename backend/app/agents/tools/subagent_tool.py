...
class SubAgentTool(BaseTool):
                ...
                for tc in response.tool_calls:
                    ...
                    if tc_name in tool_map:
                        ...
                        try:
                            ...
                            # Injetar agent_id para Filesystem tools
                            elif tc_name in (
    ...
    def _build_subagent_tools(
        ...
        try:
            # === Knowledge Base (RAG) OU File System Search ===
            # Seleção dinâmica baseada em retrieval_mode do sub-agente
            retrieval_mode = subagent_data.get("retrieval_mode", "semantic")

            if retrieval_mode == "filesystem":
                # 📂 File System Search: navega documento markdown completo
                from .filesystem_tools import FilesystemToolFactory

                fs_tools = FilesystemToolFactory.create_tools_for_agent(
                    company_id=self.company_id,
                )
                tools.extend(fs_tools)
                logger.info(
                    f"[SubAgent Tools] ✅ {len(fs_tools)} Filesystem tools para {subagent_id}"
                )
            else:
                # 🔍 RAG Híbrido (padrão): KnowledgeBaseTool inalterado
