...
/** POST — retorna o trace sanitizado da sessão sandbox. */
export async function POST(_request: NextRequest, { params }: { params: Promise<{ relayId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, trace: buildRelayTrace(session), real_action_allowed: false });
  } catch (error: any) {
    console.error('[PORTAL RELAY TRACE]', error?.message);
