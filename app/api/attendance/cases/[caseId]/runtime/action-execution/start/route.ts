...
import { startExecutionSession, safeExecutionSessionSummary } from '@/lib/attendance/action-engine';
...
/**
 * POST /api/attendance/cases/[caseId]/runtime/action-execution/start
 * Inicia a sessão de execução SANDBOX a partir do ExternalActionPlan pronto.
 * NUNCA envia externo. external_action.sent=false sempre.
 */
export async function POST(request: NextRequest, { params }: { params: Promise<{ caseId: string }> }) {
  try {
    ...
    const result = startExecutionSession(plan);
    if ('error' in result) {
      return NextResponse.json({ ok: false, error: result.error }, { status: 409 });
    }
    const sessionObj = result;

    // Persistência: sessão no dispatch_packet dry-run + resumo no caso.
    ...
    if (packet?.id) {
      ...
      await supabaseAdmin
        .from('dispatch_packets')
        .update({ metadata: { ...pmd, external_action_execution: sessionObj, execution_events: sessionObj.events.slice(-30) } })
        .eq('id', packet.id)
        .eq('company_id', companyId);
    }
    await supabaseAdmin
      .from('attendance_cases')
      .update({ metadata: mergeObject(caseRow.metadata, { external_action_execution_summary: safeExecutionSessionSummary(sessionObj) }) })
      .eq('id', caseId)
      .eq('company_id', companyId);

    console.log(`[ACTION EXEC START] case=${caseId} company=${companyId} status=${sessionObj.status} channel=${sessionObj.channel} sent=false`);
    return NextResponse.json({ ok: true, execution: sessionObj, external_action: { allowed: false, sent: false, reason: 'sandbox_42x1' } });
  } catch (error: any) {
    console.error('[ACTION EXEC START] error:', error?.message);
