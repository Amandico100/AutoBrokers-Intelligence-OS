...
export default function EmbedChat() {
  ...
  // 🔥 POLLING: Refs para exponential backoff
  ...
  useEffect(() => {
    if (!sessionId || !isOpen || sending) {
      // Limpar timer se condições não atendidas
    ...
    // Reset interval quando abre o chat ou volta de sending
      ...
      try {
        ...
        if (data.messages && data.messages.length > 0) {
          ...
          setMessages((prev) => {
            ...
            if (serverMessages.length > prev.length) {
              // Novas mensagens! Reset backoff para resposta rápida
              pollIntervalRef.current = 3000;
    ...
    // Schedule next poll recursively (allows dynamic interval)
    ...
    // Poll immediately on open, then start recursive schedule
    pollMessages().then(() => {
      scheduleNextPoll();
    });
